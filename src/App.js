...
class App extends Component {
  ...
  onClickImageItem = imageId => {
    ...
    if (id === imageId) {
      ...
      if (score === imagesList.length) {
        ...
        } while (newRandomImage.id === id)

        this.setState({randomImage: newRandomImage})
      }
    } else {
      this.setState({isGameInProgress: false})
      this.clearInterval()
  ...
  playMatchGame = () => {
    ...
    const filteredThumbnailsList = this.filteredList()
    ...
    return (
      <>
        ...
        <ul className="thumbnails-container">
          {filteredThumbnailsList.map(eachImage => (
            <ImageItem
  ...
  updateTimer = () => {
    this.setState(prevState => {
      const updatedTimer = prevState.timerInterval - 1
  ...
  showScoreCard = () => {
    ...
    return <ScoreCard score={score} playAgainButton={this.playAgainButton} />
  }
  ...
  render() {
    const {score, isGameInProgress, timerInterval} = this.state

    return (
      <div className="bg-container">
        <div>
          <NavBar score={score} timerInterval={timerInterval} />
        </div>
        {isGameInProgress ? this.playMatchGame() : this.showScoreCard()}
      </div>
    )
  }
}
